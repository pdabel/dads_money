...
class Storage:
    ...
    def save_category(self, category: Category):
        ...
        """, (
            ...
            int(category.is_income), int(category.is_tax_related),
    ...
    def save_transaction(self, transaction: Transaction):
        ...
        """, (
            ...
            transaction.date.isoformat(), transaction.payee, transaction.memo,
            str(transaction.amount), transaction.status.value, transaction.check_number,
            transaction.category_id, transaction.transfer_account_id,
            transaction.created_date.isoformat(), transaction.modified_date.isoformat()
        ))
        ...
        for split in transaction.splits:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                split.id, transaction.id, split.category_id,
                split.transfer_account_id, str(split.amount), split.memo
            ))
